...
async def cleanup_expired_sessions(ctx: dict) -> dict[str, int]:
    ...
    with Session(engine) as session:
        ...
        # Delete expired AuthSessions, but skip any still referenced by a
        # RefreshToken. An AuthSession expires after 5 minutes while its
        # RefreshToken lives ~60 days, so deleting a referenced session here
        # would raise a foreign-key violation and abort the whole cleanup.
        referenced = select(RefreshToken.auth_session_token).where(
            RefreshToken.auth_session_token.is_not(None)
        )
        stmt_sessions = (
            delete(AuthSession)
            .where(AuthSession.expires_at < now)
            .where(AuthSession.token.not_in(referenced))
        )
