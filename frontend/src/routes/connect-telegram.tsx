...
const MAX_POLL_RETRIES = 60 // 60 × 2s = 2 minutes
...
export const Route = createFileRoute("/connect-telegram")({
  ...
  beforeLoad: async () => {
    ...
    // If already linked, skip this page
    if (localStorage.getItem("needs_telegram_link") !== "1") {
      throw redirect({ to: "/" })
    }
  },
  head: () => ({
    meta: [{ title: "Connect Telegram - SolutionLab" }],
  }),
})
    ...
    const interval = window.setInterval(async () => {
      ...
      try {
        ...
        if (data.status === "verified") {
          // Update tokens if the backend issues new ones after linking
        ...
        // Short connection drops should not break the flow
      }
    }, 2000)
    ...
    <AuthLayout>
      <div className="flex flex-col gap-6">
        <div className="flex flex-col items-center gap-2 text-center">
          ...
          <h1 className="text-2xl font-semibold">{t("connect_tg_title")}</h1>
          <p className="text-sm text-muted-foreground">{t("connect_tg_sub")}</p>
        </div>
